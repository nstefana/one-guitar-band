...
The system incorporates several sound production functionalities, including that of a looper. At each repetition of the loop, the user can each each one of the following sound production mechanisms:
...
The sound of Drums and Bass can be customized to a large degree through various sound synthesis parameters and wavetables.
...
- You will be notified by email to download AppPublic.exe (which is outside the .zip folder) and use it to replace the existing one (inside your OGB folder). The application will then run without any limitations. 
